...
RtspConnection::RtspConnection(RtspServer *rtspServer, int sockfd) :
    TcpConnection(rtspServer->usageEnvironment(), sockfd),
...
void RtspConnection::handleReadBytes()
{
    ...
    switch(mMethod) {
    case OPTIONS:
        if(handleCmdOption() == false)
            goto err;
        break;
    
    case DESCRIBE:
        if(handleCmdDescribe() == false)
            goto err;
        break;
    
    case SETUP:
        if(handleCmdSetup() == false)
            goto err;
        break;
    
    case PLAY:
        if(handleCmdPlay() == false)
            goto err;
        break;
    
    case TEARDOWN:
        if(handleCmdTeardown() == false)
            goto err;
        break;
    
    case GET_PARAMETER:
        if(handleCmdGetParameter() == false)
            goto err;
        break;
...
bool RtspConnection::parseRequest()
{
    ...
    mInputBuffer.retrieveUtil(crlf + 2);
...
bool RtspConnection::parseRequest1(const char *begin, const char *end)
{
    ...
    else if(sscanf(url + 7, "%[^/]/%s", ip, suffix) == 3) {
        port = 554;
    }
    else {
        return false;
...
bool RtspConnection::parseRequest2(const char *begin, const char *end)
{
    ...
    if(mMethod == SETUP) {
        if(parseTransport(message) == false)
            return false;
...
int RtspConnection::sendMessage(void* buf, int size)
{
    ...
    ret = mOutputBuffer.write(mSocket.fd());
                                          ...
                                          uint16_t peerRtcpPort)
{
    ...
    int i;
    for(i = 0; i < 10; i++) {
        rtpSockfd = sockets::createUdpSocket();
        if(rtpSockfd < 0)
            return false;
        
        rtcpSockfd = sockets::createUdpSocket();
        if(rtcpSockfd < 0) {
            close(rtpSockfd);
            return false;
        }
...
bool RtspConnection::createRtpOverTcp(MediaSession::TrackId trackId,
                                      int sockfd, uint8_t rtpChannel)
{
